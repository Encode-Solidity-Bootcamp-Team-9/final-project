...
export class DetermineProfitability {
  ...
  async calculate(strategy: IStrategy): Promise<IProfitability> {
    const maxSellAmount: number = parseFloat(
      ethers.utils.formatUnits(
        strategy.maxSellAmount,
    ...
    for (let i = 0; i < calculationSteps; i++) {
      ...
      const amountWeWantToSellBN = ethers.utils.parseUnits(
        amountWeWantToSell.toString(),
        strategy.sellToken.decimals,
      );

      let token2AmountBN, amountAfterArbitrageBN: BigNumber;
      if (strategy.sellDex === Dex.UNISWAP) {
        token2AmountBN = await this.tradeOnUni(
          strategy.sellToken,
          strategy.buyToken,
          amountWeWantToSellBN,
        );
        amountAfterArbitrageBN = await this.tradeOnSushi(
          strategy.buyToken,
          strategy.sellToken,
          token2AmountBN,
        );
      } else {
        token2AmountBN = await this.tradeOnSushi(
          strategy.sellToken,
          strategy.buyToken,
          amountWeWantToSellBN,
        );
        amountAfterArbitrageBN = await this.tradeOnUni(
          strategy.buyToken,
          strategy.sellToken,
          token2AmountBN,
        );
      ...
      console.log(
        i +
          ': Profit ' +
          ethers.utils.formatUnits(profit, strategy.sellToken.decimals) +
          ' ' +
          strategy.sellToken.symbol +
          ' (Trade amountWeWantToSell: ' +
          amountWeWantToSell +
          ')',
      );
    ...
    strategy.firstTrade0 = ethers.utils.parseUnits(
      firstTrade0.toString(),
      strategy.sellToken.decimals,
    );
    ...
    // const gasFeesTx =
    //   await this.contracts.arbitrageContract.estimateGas.performArbitrage(
    //     strategy.buyDex,
    ...
    //     0,
    //   );
    const gasFeesTx = ethers.BigNumber.from(1);
    console.log('we got gas');
    ...
    console.table({
      ...
      'Gas cost':
        ethers.utils.formatUnits(profitability.gasCost, 'ether') + ' ETH',
      Profit:
        ethers.utils.formatUnits(profitability.profit, TOKEN_STAKING.decimals) +
        ' ' +
        TOKEN_STAKING.symbol,
    ...
    console.log(
      'GAS cost: ' +
        ...
        ' ETH (' +
        'max: ' +
        ethers.utils.formatUnits(maxGasCost, 'ether') +
        ' ETH)' +
        (gasOk ? ' OK' : '  NOT'),
    );
    console.log(
  ...
  private async tradeOnUni(
    sellToken: Token,
    buyToken: Token,
    amount: BigNumber,
  ): Promise<BigNumber> {
    return this.applySlippage(
      await this.contracts.uniQuoter.callStatic.quoteExactInputSingle(
        sellToken.address,
        buyToken.address,
        UNISWAP_POOL_FEE_TIER,
        amount.toString(),
  ...
  private async tradeOnSushi(
    ...
    return this.applySlippage(
      (
        await this.contracts.sushiRouter.getAmountsOut(amount.toString(), [
          sellToken.address,
          buyToken.address,
        ])
      )[1],
  ...
  private applySlippage(num: BigNumber, decimals: number): BigNumber {
    let withSlippage =
      parseFloat(ethers.utils.formatUnits(num, decimals)) * (1 - SLIPPAGE);
